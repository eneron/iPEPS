...
tensor_AB_gate=ncon({tensor_B,tensor_A,gate_tensor},...
    {[2,3,-6,-7,-8],[1,-3,-4,3,-2],[-1,-5,1,2]});
tensor_Theta=ncon({tensor_AB_gate,tensor_Env},...
    {[-1,3,1,2,-5,4,5,6],[1,-2,2,-3,3,-4,4,-6,5,-7,6,-8]});
...
tensor_B_updated=tensor_B_updated/max(abs(tensor_B_updated(:)));
    ...
    function [fn,grad_fn]=f_grad_f(vectorized_AB)
        ...
        % functions like herel the nested function (inside) can used parent
        % variables; parent functions can use the variables inside the
        % children functions unless the variables is the output of the
        % children functions.
        
        % vectorized_AB=[real(vector_A);imag(vector_A);real(vector_B);imag(vector_A)]
        ...
        {[-1,-5,-2,-6,-4,-8,1,2,3,4,5,6],[-3,1,3,5,-7,2,4,6]});
        matrix_N_A=kron(...
            transpose(reshape(tensor_E_A,[BOND_DIM^4,BOND_DIM^4])),...
            eye(PHYS_DIM));
%         matrix_M_A=kron(eye(PHYS_DIM),...
%             reshape(tensor_E_A,[BOND_DIM^4,BOND_DIM^4]));
        
        tensor_G_A=ncon({tensor_Theta,conj(tensor_B_prime)},...
            {[-1,-2,-3,-5,1,2,3,4],[1,-4,2,3,4]});
        ...
        {[1,2,3,4,5,6,-2,-6,-3,-7,-4,-8],[1,3,-1,5,2,4,-5,6]});
        matrix_N_B=kron(...
            transpose(reshape(tensor_E_B,[BOND_DIM^4,BOND_DIM^4])),...
            eye(PHYS_DIM));
%         matrix_M_B=kron(eye(PHYS_DIM),...
%             reshape(tensor_E_B,[BOND_DIM^4,BOND_DIM^4]));
        
        tensor_G_B=ncon({tensor_Theta,conj(tensor_A_prime)},...
            {[1,2,3,4,-1,-3,-4,-5],[1,2,3,-2,4]});
